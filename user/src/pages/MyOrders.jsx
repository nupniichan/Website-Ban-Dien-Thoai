  ...
  return (
    <div className="flex flex-col md:flex-row min-h-[690px] bg-gray-100 p-5 mb-20">
      ...
      <div className="flex-1 ml-0 md:ml-10 bg-white p-6 rounded-lg shadow-lg overflow-y-auto">
        <h1 className="text-2xl font-semibold mb-6 text-blue-800">Lịch sử giao dịch</h1>
        {loading ? (
          <p>Loading...</p>
        ) : error ? (
              ...
              <>
                <ul className="grid gap-5">
                  {currentOrders.map((order) => {
                    ...
                    const canCancel = !['Đã hủy', 'Đã giao hàng', 'Đang giao hàng', 'Đã hoàn tiền'].includes(order.status);
                    ...
                    return (
                      ...
                      >
                        <div className="flex justify-between items-start">
                          <div>
                            <p className="text-lg font-semibold text-gray-800">Mã đơn hàng: #{order.id}</p>
                            <p className="text-lg text-gray-600">Tổng tiền: {formatCurrency(order.totalAmount)}</p>
                            <p className="text-md text-gray-500">Ngày đặt: {formatDate(order.orderDate)}</p>
                          </div>
                          <div className="flex flex-col gap-2">
                            <div className={`px-4 py-2 rounded-full border ${statusStyle.bg} ${statusStyle.text} ${statusStyle.border}`}>
                              {order.status}
                            </div>
                          </div>
                        </div>
                    ...
                    <div className="p-6 bg-white rounded-lg">
                      <div className="grid grid-cols-2 gap-4 mb-6">
                        ...
                        <div>
                          ...
                          <p className={`font-semibold ${getStatusStyle(selectedOrder.status).text} 
                          ...
                          {selectedOrder.items.map((item) => {
                            ...
                            return (
                              <tr key={item.productId} className="hover:bg-gray-200 transition-colors duration-200">
                                ...
                                <td className="border px-4 py-2">{product ? formatCurrency(product.price) : 'N/A'}</td>
                              </tr>
                            );
                          })}
                        </tbody>
                ...
                <Modal
                  ...
                  footer={[
                    ...
                    <Button key="submit" type="primary" danger onClick={handleCancelSubmit}>
                      Xác nhận hủy
                    </Button>,
                  ]}
                >
